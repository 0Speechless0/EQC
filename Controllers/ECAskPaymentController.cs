...
{
    [SessionFilter]
    public class ECAskPaymentController : Controller
    {//工程變更 - 估驗請款
        ECAskPaymentService iService = new ECAskPaymentService();

        //日期清單
        public JsonResult GetDateList(int id)
        {
            List<ECAskPaymentHeaderVModel> dList = iService.GetDateList<ECAskPaymentHeaderVModel>(id);
            return Json(new
            {
                result = 0,
                items = dList
        ...
        public JsonResult AddAskPayment(int id, DateTime tarDate)
        {
            ...
            if (tarDate.Subtract(engChange.StartDate.Value).Days < 0)
            {
                ...
                {
                    result = -1,
                    msg = String.Format("日期{0}之前的請款, 請至 進度管理 頁面作業", engChange.StartDate.Value.ToString("yyyy-M-d"))
                });
            }
            ...
            List<ECAskPaymentEngVModel> tenders = iService.GetEngItem<ECAskPaymentEngVModel>(id);
            if (tenders.Count == 0)
            {
                return Json(new
                {
            ...
            List<ECAskPaymentHeaderVModel> dates = iService.GetDateList<ECAskPaymentHeaderVModel>(id);
            //List<ECAskPaymentHeaderVModel> dates = iService.CheckDate<ECAskPaymentHeaderVModel>(id, tarDate);
            if (dates.Count == 0)
            {//工變後,首次請款
                //進度管理, 請款紀錄
                List<EPCAskPaymentHeaderVModel> dList = new AskPaymentService().GetDateList<EPCAskPaymentHeaderVModel>(id);
                DateTime lastDate = engChange.StartDate.Value.AddDays(-1);
                if (dList.Count == 0)
                {
                    return Json(new
                    {
                        result = -1,
                        msg = String.Format("請至 進度管理, 完成 {0} 請款作業", lastDate.ToString("yyyy-M-d"))
                    });
                }
                else
                {
                    EPCAskPaymentHeaderVModel m = dList[0];
                    if (m.APState != 1 || m.APDate.Subtract(lastDate).Days != 0)
                    {
                        return Json(new
                        {
                            result = -1,
            ...
            {
                DateTime lastDate = dates[0].APDate;
                if (tarDate.Subtract(lastDate).Days <= 0)
                {
                    return Json(new
                    {
                        result = -1,
                ...
                foreach (EPCProgressEngChangeListVModel ec in engChanges)
                {
                    ...
                    if (tarDate.Subtract(eDate).Days <= 0)
                    {
                        if (lastDate.Subtract(ec.StartDate.Value).Days < 0)
                        {
                            return Json(new
                            {
                                result = -1,
                                msg = String.Format("請先完成 {0} 請款作業", ec.StartDate.Value.AddDays(-1).ToString("yyyy-M-d"))
                            });
                        }
                        break;
                    }
            ...
            List<EC_SupDailyDateModel> dailys = iService.CheckDailyDate<EC_SupDailyDateModel>(id, tarDate);
            if (dailys.Count == 0)
            {
                return Json(new
                {
            ...
            EC_SupDailyDateModel daily = dailys[0];
        ...
        //更新
        public JsonResult APSave(List<EC_AskPaymentPayItemModel> items)
        {
        ...
        {
            List<ECAskPaymentHeaderVModel> hList = iService.GetAskPaymentHeader<ECAskPaymentHeaderVModel>(id);
            if (hList.Count != 1)
            {
                return Json(new
                {
            ...
            ECAskPaymentHeaderVModel apHeader = hList[0];
            List<ECAskPaymentPayItemVModel> apPayItemList = iService.GetAskPaymentList<ECAskPaymentPayItemVModel>(apHeader.Seq);
            //前期
            ECAskPaymentHeaderVModel apPreviousHeader = new ECAskPaymentHeaderVModel() { CurrentAccuAmount = 0, Period = 0 };
            List<ECAskPaymentPayItemVModel> apPreviousPayItemList = new List<ECAskPaymentPayItemVModel>();
            if (apHeader.Period == 1)
            {
                apPreviousPayItemList = iService.GetProphaseAskPaymentList<ECAskPaymentPayItemVModel>(apHeader.EngMainSeq, apHeader.Seq);
            } else if (apHeader.Period>1)
            {
                hList = iService.GetAskPaymentHeader<ECAskPaymentHeaderVModel>(apHeader.EngMainSeq, apHeader.Period-1);
                if (hList.Count == 1)
                {
                    apPreviousHeader = hList[0];
                    apPreviousPayItemList = iService.GetAskPaymentList<ECAskPaymentPayItemVModel>(apPreviousHeader.Seq);
                }
            }
            ...
            List<ECAskPaymentPayItemListVModel> apPayItems = new List<ECAskPaymentPayItemListVModel>();
            ECAskPaymentPayItemListVModel item = null;
            string mKey = "";
            for (int i=0; i< apPayItemList.Count;i++)
            {
                ECAskPaymentPayItemVModel m = apPayItemList[i];
                if (m.level==1)
                {
                    item = new ECAskPaymentPayItemListVModel();
            ...
            List<ECAskPaymentEngVModel> items = iService.GetEngItem<ECAskPaymentEngVModel>(apHeader.EngMainSeq);
            if (items.Count != 1)
            {
                return Json(new
                {
            ...
            ECAskPaymentEngVModel eng = items[0];
        ...
        private ActionResult CreateInvoiceExcel(ECAskPaymentEngVModel eng, ECAskPaymentHeaderVModel apHeader, ECAskPaymentHeaderVModel apPreviousHeader, List<AskPaymentPayItemModel> miscList, List<ECAskPaymentPayItemListVModel> apPayItems)
        {
            ...
            try
            {
                ...
                foreach (ECAskPaymentPayItemListVModel m in apPayItems)
                {//僅有細項
            ...
            catch (Exception e)
            {
                ...
                BaseService.log.Info("ECAskPaymentController.CreateInvoiceExcel: " + e.Message);
                return Json(new
                {
                    result = -1,
                    message = "Excel 製表失敗"
        ...
        //總表
        private void PayitemInvoiceHeaderSheet(Worksheet sheet, ECAskPaymentEngVModel eng, ECAskPaymentHeaderVModel apHeader, ECAskPaymentHeaderVModel apPreviousHeader)
        {
        ...
        //估0
        private void PayitemInvoice0(Worksheet sheet, List<ECAskPaymentPayItemListVModel> apPayItems)
        {
            if (sheet == null) return;
            ...
            foreach (ECAskPaymentPayItemListVModel m in apPayItems)
            {
                ...
                foreach (ECAskPaymentPayItemVModel item in m.subItems)
                {
                    if (item.ItemType != -1)
                    {
        ...
        //估1..n
        private void PayitemInvoiceSub(Worksheet sheet, ECAskPaymentPayItemListVModel apPayItems)
        {
            if (sheet == null) return;
            ...
            int row = 10;
            decimal totalPreviousAccuAmount = 0, totalPreviousSchAmount = 0, totalAccuAmount = 0, total = 0, totalSchAmount = 0;
            foreach (ECAskPaymentPayItemVModel m in apPayItems.subItems)
            {
                if (m.ItemType == -1)
                {
        ...
        public ActionResult DownloadAdj(int id)
        {
            ...
            ECAskPaymentHeaderVModel askPaymentHeader = hList[0];

            List<ECAskPaymentEngVModel> items = iService.GetEngItem<ECAskPaymentEngVModel>(askPaymentHeader.EngMainSeq);
            if (items.Count != 1)
            {
                return Json(new
                {
        ...
        private ActionResult CreateAdjExcel(ECAskPaymentEngVModel eng, ECAskPaymentHeaderVModel askPaymentHeader, List<AskPaymentPayItemModel> miscList)
        {
        ...
        //payitem
        private void PayitemAdjSheet(Worksheet sheet, ECAskPaymentEngVModel eng, ECAskPaymentHeaderVModel askPaymentHeader, List<AskPaymentPayItemModel> spList)
        {
            if (sheet == null) return;
        ...
        public JsonResult UploadAccu(int id)
        {
            ...
            if (file.ContentLength > 0)
            {
                ...
                List<EC_AskPaymentPayItemModel> apList = iService.GetList<EC_AskPaymentPayItemModel>(id);
                if (apList.Count == 0)
                {
                    return Json(new
                    {
        ...
        private int readExcelData(string filename, ECAskPaymentEngVModel eng, ECAskPaymentHeaderVModel askPaymentHeader, List<EC_AskPaymentPayItemModel> items)
        {
            Microsoft.Office.Interop.Excel.Application appExcel = null;
            Workbook workbook = null;
            //開啟 Excel 檔案
            try
            {
                ...
                //檢查 PayItem 是否一致
                int row = 6;
                foreach (EC_AskPaymentPayItemModel m in items)
                {

...
{
    public class CarbonEmissionMaterialService : BaseService
    {//材料 碳排係數維護

        //批次處裡
        public void ImportData(List<CarbonEmissionMaterialModel> items, ref int iCnt, ref int uCnt, ref string errCnt)
        {
            SqlCommand cmd;
            string sql;
            ...
            {
                Null2Empty(m);
                sql = @"SELECT Seq FROM CarbonEmissionMaterial
                        where Kind=@Kind
        ...
        //新增
        public int AddRecord(CarbonEmissionMaterialModel m)
        {
            Null2Empty(m);
            string sql = @"
            insert into CarbonEmissionMaterial (
                Kind,
                NameSpec,
                KgCo2e,
                Unit,
            ...
            {
                log.Info("CarbonEmissionMaterialService.AddRecord: " + e.Message);
                return -1;
            }
        }
        //更新
        public int UpdateRecord(CarbonEmissionMaterialModel m)
        {
            Null2Empty(m);
            string sql = @"
            update CarbonEmissionMaterial set 
                IsDel=0,
                Kind = @Kind,
                NameSpec = @NameSpec,
                KgCo2e = @KgCo2e,
            ...
            {
                log.Info("CarbonEmissionMaterialService.UpdateRecord: " + e.Message);
                return -1;
            }
        }
        //清單
        ...
        {
            string sql = @"SELECT max(ModifyTime) itemDT FROM CarbonEmissionMaterial";
            SqlCommand cmd = db.GetCommand(sql);
        ...
        public int DelRecord(int seq)
        {
            ...
            try
            {
                //sql = @"delete from CarbonEmissionMaterial where Seq=@Seq";
                sql = @"
                update CarbonEmissionMaterial set
                    IsDel=1,
                    ModifyTime = GetDate(),
            ...
            catch (Exception e)
            {
                ...
                log.Info("CarbonEmissionMaterialService.DelRecord: " + e.Message);
                log.Info(sql);
                return -1;
            }
        }

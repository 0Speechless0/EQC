...
{
    public class CarbonEmissionFuelService : BaseService
    {//燃料 碳排係數維護

        //批次處裡
        public void ImportData(List<CarbonEmissionFuelModel> items, ref int iCnt, ref int uCnt, ref string errCnt)
        {
            SqlCommand cmd;
            string sql;
            ...
            {
                Null2Empty(m);
                sql = @"SELECT Seq FROM CarbonEmissionFuel
        ...
        //新增
        public int AddRecord(CarbonEmissionFuelModel m)
        {
            Null2Empty(m);
            string sql = @"
            insert into CarbonEmissionFuel (
                NameSpec,
                KgCo2e,
                Unit,
                Memo,
            ...
            try
            {
                ...
                string sql1 = @"SELECT IDENT_CURRENT('CarbonEmissionFuel') AS NewSeq";
            ...
            {
                log.Info("CarbonEmissionFuelService.AddRecord: " + e.Message);
                return -1;
            }
        }
        //更新
        public int UpdateRecord(CarbonEmissionFuelModel m)
        {
            Null2Empty(m);
            string sql = @"
            update CarbonEmissionFuel set 
                IsDel=0,
                NameSpec = @NameSpec,
                KgCo2e = @KgCo2e,
                Unit = @Unit,
            ...
            {
                log.Info("CarbonEmissionFuelService.UpdateRecord: " + e.Message);
                return -1;
            }
        }
        //清單
        ...
        {
            string sql = @"SELECT max(ModifyTime) itemDT FROM CarbonEmissionFuel";
            SqlCommand cmd = db.GetCommand(sql);
        ...
        public int DelRecord(int seq)
        {
            ...
            try
            {
                //sql = @"delete from CarbonEmissionFuel where Seq=@Seq";
                sql = @"
                update CarbonEmissionFuel set
                    IsDel=1,
                    ModifyTime = GetDate(),
            ...
            catch (Exception e)
            {
                ...
                log.Info("CarbonEmissionFuelService.DelRecord: " + e.Message);
                log.Info(sql);
                return -1;
            }
        }

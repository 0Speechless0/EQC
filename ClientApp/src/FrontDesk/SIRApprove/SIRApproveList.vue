                    ...
                    <tr v-for="(item, index) in items" v-bind:key="item.subEngNameSeq">
                        ...
                        <td>
                            <div class="row justify-content-center m-0">
                                <button v-on:click.stop="onApproved(item)" v-bind:disabled="item.hasUnderReview==0" v-bind:class="{'btn-color2':(item.hasUnderReview>0), 'btn-color3':(item.hasUnderReview==0)}" class="btn-block mx-2 btn" title="同意">同意</button>
                            </div>
                            </td><td>
                            <div class="row justify-content-center m-0">
                                <button v-on:click.stop="onDisagree(item)" v-bind:disabled="item.hasUnderReview==0" v-bind:class="{'btn-color2':(item.hasUnderReview>0), 'btn-color3':(item.hasUnderReview==0)}" class="btn-block mx-2 btn" title="退回">退回</button>
                            </div>
                        </td>
                                 <td>
                                     <div class="row justify-content-center m-0">
                                         <a v-on:click="getdownloaditem(item,1)" href="#" class="mx-2 btn btn-block btn-outline-secondary" title="下載" data-toggle="modal" data-target="#wordModal">doc下載</a>
                                         <a v-on:click="getdownloaditem(item,2)" href="#" class="mx-2 btn btn-block btn-outline-secondary" title="下載" data-toggle="modal" data-target="#wordModal">pdf下載</a>
                                         <a v-on:click="getdownloaditem(item,3)" href="#" class="mx-2 btn btn-block btn-outline-secondary" title="下載" data-toggle="modal" data-target="#wordModal">odt下載</a>
                                         <!-- 大圖 -->
...
<script>
    export default {
        ...
        methods: {
            //取得使用者單位資訊
            getUserUnit() {
                window.myAjax.post('/SIRApprove/GetUserUnit')
                    .then(resp => {
            ...
            async getSelectYearOption() {
                const { data } = await window.myAjax.post('/SIRApprove/GetYearOptions');
            ...
            async onYearChange(event) {
                ...
                const { data } = await window.myAjax.post('/SIRApprove/GetUnitOptions', { year: this.selectYear });
            ...
            async onUnitChange(unitSeq) {
                ...
                this.selectSubEngName = -1;
                this.selectSubEngNameItems = [];
                const { data } = await window.myAjax.post('/SIRApprove/GetSubUnitOptions', { year: this.selectYear, parentSeq: unitSeq });
            ...
            getEngNameList() {
                ...
                window.myAjax.post('/SIRApprove/GetEngNameList'
                    , {
            ...
            async getSubEngNameList() {
                ...
                await window.myAjax.post('/SIRApprove/GetSubEngNameList'
                    , {
                        engMain: targetId,
                    })
                    .then(resp => {
            ...
            getList() {
                ...
                window.myAjax.post('/SIRApprove/GetListLightly'
                    ,{
            ...
            download(item) {
                window.myAjax.get('/SIRApprove/SIRDownload?seq=' + this.downloaditem.subEngNameSeq + '&filetype=' + this.downloadtype, { responseType: 'blob' })
                    .then(resp => {
            ...
            downloadone(item, downloaditem) {
                ...
                window.myAjax.get('/SIRApprove/SIROneDownloadApprove?seq=' + this.downloaditem.subEngNameSeq + '&items=' + this.downloaditems + '&downloaditem=' + downloaditem + '&num=' + num + '&filetype=' + this.downloadtype, { responseType: 'blob' })
                    .then(resp => {
                        const blob = new Blob([resp.data]);
                        const contentType = resp.headers['content-type'];
                        if (contentType.indexOf('application/json') >= 0) {
            ...
            onDisagree(item) {
                window.myAjax.post('/SIRApprove/Disagree', { seq: item.subEngNameSeq, sdName: item.SupervisorDirector })
                    .then(resp => {
            ...
            onApproved(item) {
                window.myAjax.post('/SIRApprove/Approved', { seq: item.subEngNameSeq, sdName: item.SupervisorDirector })
                    .then(resp => {
                        if (resp.data.result == 0) {
                            this.getList();
                        }
            ...
            getdownloaditem(item, type) {
                ...
                window.myAjax.post('/SIRApprove/GetSIRlistApprove', { seq: item.subEngNameSeq })
                    .then(resp => {

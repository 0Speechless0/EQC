    ...
    export default {
        ...
        methods: {
            ...
            getSubEngNameList() {
                ...
                window.myAjax.post('/SIRApprove/GetSubEngNameList'
                    , {
                        engMain: this.selectEngName,
                    })
                    .then(resp => {
            ...
            getList() {
                if (this.selectYear == '' || this.selectUnit == '' || this.selectSubUnit == '') return;
                window.myAjax.post('/SIRApprove/GetList'
                    ,{
                        year: this.selectYear,
                        unit: this.selectUnit,
                        subUnit: this.selectSubUnit,
            ...
            downloadone(item, downloaditem) {
                ...
                window.myAjax.get('/SIRApprove/SIROneDownload?seq=' + this.downloaditem.subEngNameSeq + '&items=' + this.downloaditems + '&downloaditem=' + downloaditem + '&num=' + num + '&filetype=' + this.downloadtype, { responseType: 'blob' })
                    .then(resp => {
                        const blob = new Blob([resp.data]);
                        const contentType = resp.headers['content-type'];
                        if (contentType.indexOf('application/json') >= 0) {
            ...
            getdownloaditem(item, type) {
                ...
                window.myAjax.post('/SIRApprove/GetSIRlist', { seq: item.subEngNameSeq })
                    .then(resp => {

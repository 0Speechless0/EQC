<template>
    <div class="">
        ...
        <div class="form-group">
            ...
            <textarea v-model="ncr.MissingItem" @change="onDataChange" disabled="true" id="NCRdescription01" class="form-control mb-3" rows="5"></textarea>
        </div>
        <div class="form-group">
            <label for="NCRdescription02" class="m-2">二、原因分析</label>
            <textarea v-model="ncr.CauseAnalysis" @change="onDataChange" disabled="true" id="NCRdescription02" class="form-control mb-3" rows="5"></textarea>
        </div>
        <label class="m-2">三、矯正(改善)及預防措施(品管人員提出)</label>
        <div class="form-group">
            <label for="NCRdescription03-1" class="m-2">(一)矯正措施</label>
            <textarea v-model="ncr.CorrectiveAction" @change="onDataChange" disabled="true" id="NCRdescription03-1" class="form-control mb-3" rows="5"></textarea>
        </div>
        <div class="form-group">
            <label for="NCRdescription03-2" class="m-2">(二)預防措施</label>
            <textarea v-model="ncr.PreventiveAction" @change="onDataChange" disabled="true" id="NCRdescription03-2" class="form-control mb-3" rows="5"></textarea>
        </div>
        <div class="form-group">
            <label for="NCRdescription04" class="m-2">四、矯正預防措施與改善結果</label>
            <textarea v-model="ncr.CorrPrevImproveResult" @change="onDataChange" disabled="true" id="NCRdescription04" class="form-control mb-3" rows="5"></textarea>
        </div>
        <h2>審核結果（由原檢查人員認可）</h2>
        <div class="form-inline my-2">
            <label class="m-2">改善審核結果：</label>
            <select v-model="ncr.ImproveAuditResult" @change="onImproveAuditResultChange" class="form-control">
        ...
        <div class="form-inline">
            ...
            <div class="custom-control custom-radio mx-2">
                <input v-model="ncr.CanClose" @change="onDataChange" v-bind:disabled="ncr.ImproveAuditResult==1" type="radio" id="NCRagreeCaseClosedYes" class="custom-control-input" name="ncrCanClose" value="true">
                <label class="custom-control-label" for="NCRagreeCaseClosedYes">是</label>
            </div>
            <div class="custom-control custom-radio mx-2">
                <input v-model="ncr.CanClose" @change="onDataChange" v-bind:disabled="ncr.ImproveAuditResult==1" type="radio" id="NCRagreeCaseClosedNo" class="custom-control-input" name="ncrCanClose" value="false">
                <label class="custom-control-label" for="NCRagreeCaseClosedNo">否</label>
            </div>
        </div>
        <div class="row justify-content-center">
            <div class="col-12 col-lg-6 mt-3">
                <button v-on:click="onUpdateReport" v-bind:disabled="ncr.FormConfirm!=1" v-bind:class="{'btn-color1':(ncr.FormConfirm==1), 'btn-color3':(ncr.FormConfirm!=1)}" role="button" class="btn btn-shadow btn-block">
                    儲存
                </button>
            </div>
            <!-- div v-if="ncr.Seq != null" class="col-6 col-lg-3 mt-3">
                <button v-on:click="onFormConfirm" v-bind:disabled="ncr.FormConfirm!=1" v-bind:class="{'btn-color1':(ncr.FormConfirm==1), 'btn-color3':(ncr.FormConfirm!=1)}" role="button" class="btn btn-shadow btn-block">
                    確認
                </button>
            </div -->
        </div>
    ...
    export default {
        ...
        methods: {
            ...
            getNCR() {
                ...
                this.ncr = { };
                window.myAjax.post('/SIRIApprove/GetNCR', { rec: this.si })
                    .then(resp => {
            ...
            onDatePicketChange(ctx, mode) {
                ...
                if (ctx.selectedDate != null) {
                    ...
                    if (mode == 'chsProcessTrackDate')
                        this.ncr.chsProcessTrackDate = dd;
                }
            },
            //中曆轉西元
            toYearDate(dateStr) {
            ...
            onImproveAuditResultChange() {
                this.ncr.CanClose = (this.ncr.ImproveAuditResult == 1);
            },
            onUpdateReport() {
                ...
                window.myAjax.post('/SIRIApprove/UpdateNCRApprove', { ncr: this.ncr })
                    .then(resp => {
                        if (resp.data.result == 0) {
                            this.fResultChange = false;
                            this.ncr.FormConfirm = resp.data.formConfirm
                        }
                        alert(resp.data.message);
                    })
                    .catch(err => {
            ...
            onFormConfirm() {
                ...
                if (confirm('資料確認後將無法修改, 是否 確認此NCR程序追蹤改善表?')) {
                    window.myAjax.post('/SIRIApprove/NCRConfirm', { id: this.ncr.Seq })
                        .then(resp => {
                            if (resp.data.result == 0) {
        ...
        async mounted() {
            console.log('mounted()審查 NCR程序追蹤改善表');
            var d = new Date();

        ...
        <form @submit.prevent>
            ...
            <div v-show="si.Seq>0" class="tab-content">
                ...
                <div id="menu03" class="tab-pane">
                    <!-- 改善照片 -->
                    <PhotoGroup v-if="selectTab=='Photo'" v-bind:engMain="engMain.Seq" v-bind:si="si"></PhotoGroup>
                </div>
            </div>
...
<script>
    export default {
        ...
        components: {
            ...
            PhotoGroup: require('../SamplingInspectionRecImprove/PhotoGroup.vue').default,
        ...
        methods: {
            ...
            getEngItem() {
                this.step = 2;
                this.engMain = {};
                window.myAjax.post('/SIRIApprove/GetEngItem', { id: this.targetId })
                    .then(resp => {
            ...
            async getRecCheckTypeOption() {
                ...
                const { data } = await window.myAjax.post('/SIRIApprove/GetRecCheckTypeOption', { id: this.targetId });
            ...
            onSearchCheckTypeChange() {
                ...
                window.myAjax.post('/SIRIApprove/GetRecOptionByCheckType', { constructionSeq: this.targetId, checkTypeSeq: this.searchCheckType })
                    .then(resp => {
            ...
            getRecHeader(id) {
                if (id == -1) return;
                this.hasNCR = false;
                window.myAjax.post('/SIRIApprove/GetRec', { recSeq: id })
                    .then(resp => {
                        if (resp.data.result == 0) {
        ...
        async mounted() {
            console.log('mounted() 缺失改善審查-編輯' + window.location.href);

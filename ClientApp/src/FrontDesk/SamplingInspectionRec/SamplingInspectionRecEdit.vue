    ...
    <div>
        ...
        <form @submit.prevent>
            <div class="justify-content-between" v-show="!QisShow">
                ...
                <comm-pagination ref="pagination" :pageRecordList="[5]" :recordTotal="recordTotal" v-on:onPaginationChange="onPaginationChange"></comm-pagination>
            ...
            <div v-if="!NisShow">
                    ...
                    <table v-show="QisShow || !isNew" class="table table2 min720 my-0" border="0">
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    <div class="form-inline">
                                        <select v-model="si.EngConstructionSeq" @change="onEngConstructionChange" v-bind:disabled="!QisShow" class="form-control mb-2 mr-sm-2">
                                            <option v-for="option in engConstructions" v-bind:value="option.Value" v-bind:key="option.Value">
                                                {{ option.Text }}
                                            </option>
                                        </select>
                                        <label class="m-1">檢查日期</label>
                                        <b-input-group class="mydatewidth">
                                            <input v-bind:value="chsRecDate" @change="onDateChange(chsRecDate, $event)" type="text" class="form-control" placeholder="yyy/mm/dd" v-bind:disabled="!QisShow">
                                        ...
                                        <select v-if="fDebug" v-model="si.CCRCheckType1" @change="onCheckTypeChange" class="form-control mb-2 mr-sm-2" disabled>
                                        ...
                                        <select v-if="constCheckMode==1 || constCheckMode==4" v-model="si.CCRCheckFlow" @change="onCheckFlowChange" class="form-control mb-2 mr-sm-2" v-bind:disabled="!QisShow">
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="form-inline">
                                        <input v-model="si.CCRPosDesc" @change="onResultChange" type="text" class="form-control col-10" placeholder="輸入位置描述" v-bind:disabled="!QisShow">
                                        <button v-if="isNew  && QisShow" v-on:click="onNewRec" role="button" class="btn btn-outline-secondary btn-xs mx-1 col-1">
                                            新增
                                        </button>
                                    </div>
                ...
                <div v-if="!isNew">
                    <div class="row">
                        <div class="col col-lg-4">
                            <label class="m-1 small-green row">
                                ...
                                <div class="col-12 pl-0">
                                    * 點 <button class="btn btn-outline-secondary btn-xs sharp m-1" disabled><i class="fas fa-copy"></i></button> 後，可帶入前次抽查紀錄，需修改資料後方可儲存
                                </div>
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th style="width:200px" v-if="!isNew">
                                        <button @click="getRecResultHistory" class="btn btn-outline-secondary btn-xs sharp m-1" title="複製前次抽查紀錄"><i class="fas fa-copy"></i></button>
                                        實際抽查情形
                                    </th>
                            ...
                            <tbody>
                                <tr v-for="(item, index) in items" v-bind:key="item.ControllStSeq">
                                    ...
                                    <td v-if="item.CheckFields>=0 && item.rowSpanStd1>0" v-bind:rowspan="item.rowSpanStd1">
                                        {{item.Stand1}}
                                        <button v-if="item.itemType==4 && si.FormConfirm==0 && (item.CCRRealCheckCond==null || item.CCRRealCheckCond.length==0)"
                                                @click="openSetStd(item)" data-toggle="modal" data-target="#setStdModal" role="button" class="btn btn-color11-3 btn-xs mx-1">
                                            抽查標準
                                        </button>
                                    </td>
                                    <td v-if="!isNew && item.rowSpan>0" v-bind:rowspan="item.rowSpan">
                                        <div contenteditable="true" aria-multiline="true">
                                            <textarea v-model.trim="item.CCRRealCheckCond" v-bind:rows="item.rowSpan==1 ? 3 : 6"
                                                      v-bind:class="[item.changed ? '' : 'mustEdit']" @change="onResultChange(item)"
                                                      v-on:keyup="onInputKeyup(index, item)"
                                    ...
                                    <td v-if="!isNew && item.rowSpan>0" v-bind:rowspan="item.rowSpan">
                                        <div>
                                            ...
                                            <div v-show="constCheckMode==1 && item.CCRCheckResult==2" class="custom-control custom-checkbox">
                                        ...
                                        <div>
                                            <textarea v-model="item.RecResultRemark" class="form-control" maxlength="150"> </textarea>
                                        </div>
                                    </td>
                                    <!--
                                <td v-if="!isNew && item.rowSpan>0" v-bind:rowspan="item.rowSpan">
                    ...
                    <div class="form-inline mb-3">
                        ...
                        <button v-on:click="uploadPhotoModal(selectStdItem)" v-show="si.FormConfirm<1" class="btn btn-color11-2 btn-xs mx-1">
                            <i class="fas fa-upload"></i> 上傳照片
                        </button>
                    </div>
                    <PhotoList1 v-bind:engMain="engMain.Seq" v-bind:si="si" v-bind:ctlSeq="selectStdItem.ControllStSeq" ref="photoList"></PhotoList1>
                </div>
            </div>
        </form>
        ...
        <div v-if="!isNew" class="row justify-content-center">
            ...
            <button v-on:click="onDelRec" v-bind:disabled="si.FormConfirm>0 && userInfo.RoleSeq != 1" role="button" class="btn btn-color9-1 btn-xs mx-1">
            ...
            <div class="modal-dialog modal-xl modal-dialog-centered " style="width:600px">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" id="closeSetStdModal" class="btn btn-secondary" data-dismiss="modal" aria-label="Close">
                            取消
                        </button>
                    </div>
                </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            //s20230302 複製資料
            setItemData() {
                for (var i = 0; i < this.itemsHistory.length; i++) {
                    let item = this.itemsHistory[i];
                    if (item.rowSpan > 0 && !window.comm.stringEmpty(item.CCRRealCheckCond) && this.items[i].CCRRealCheckCond != item.CCRRealCheckCond) {
                        this.items[i].CCRRealCheckCond = item.CCRRealCheckCond;

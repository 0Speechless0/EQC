<template>
    <div>
        <form class="form-group">
            <div class="form-row">
                <div class="col-12 col-sm-2 col-lg-1 mt-1">
                ...
                <div class="col-12 col-sm-5 col-lg-2 mt-2">
        ...
        <div class="table-responsive">
            <table class="table table1 rwd-table" border="0">
                <thead>
                    <tr>
                        <th class="sort">項次</th>
                ...
                <tbody>
                    <tr v-for="(item, index) in items" v-bind:key="item.subEngNameSeq">
                        <td data-th="項次">{{pageRecordCount*(pageIndex-1)+index+1}}</td>
                        <td data-th="工程編號">{{item.EngNo}}</td>
                        <td data-th="工程名稱">{{item.EngName}}</td>
                        <td data-th="執行機關">{{item.ExecUnit}}</td>
                        <td data-th="監造單位">{{item.SupervisorUnitName}}</td>
                        <td data-th="分項工程名稱">{{item.subEngNameSeq}},{{item.subEngName}}</td>
                        <td data-th="缺失個數">{{item.missingCount}}</td>
                        <td data-th="編輯">
                        ...
                        <td data-th="施工抽查紀錄表">
                            <div class="row justify-content-center m-0">
                                <a v-on:click.stop="download(item)" href="#" class="mx-2 btn btn-block btn-outline-secondary" title="下載">下載</a>
                            </div>
                        </td>
                    </tr>
                </tbody>
...
<script>
    export default {
        ...
        methods: {
            //取得使用者單位資訊
            getUserUnit() {
                window.myAjax.post('/MSamplingInspectionRec/GetUserUnit')
                    .then(resp => {
            ...
            async getSelectYearOption() {
                const { data } = await window.myAjax.post('/MSamplingInspectionRec/GetYearOptions');
            ...
            async onYearChange(event) {
                ...
                const { data } = await window.myAjax.post('/MSamplingInspectionRec/GetUnitOptions', { year: this.selectYear });
            ...
            async onUnitChange(unitSeq) {
                ...
                this.selectSubEngName = -1;
                this.selectSubEngNameItems = [];
                const { data } = await window.myAjax.post('/MSamplingInspectionRec/GetSubUnitOptions', { year: this.selectYear, parentSeq: unitSeq });
            ...
            getEngNameList() {
                ...
                window.myAjax.post('/MSamplingInspectionRec/GetEngNameList'
                    , {
            ...
            getSubEngNameList() {
                ...
                window.myAjax.post('/MSamplingInspectionRec/GetSubEngNameList'
                    , {
                        engMain: this.selectEngName,
                    })
                    .then(resp => {
            ...
            getList() {
                if (this.selectYear == '' || this.selectUnit == '' || this.selectSubUnit == '') return;
                window.myAjax.post('/MSamplingInspectionRec/GetList'
                    ,{
                        year: this.selectYear,
                        unit: this.selectUnit,
                        subUnit: this.selectSubUnit,
            ...
            download(item) {
                window.myAjax.get('/MSamplingInspectionRec/SIRDownload?seq=' + item.subEngNameSeq, { responseType: 'blob' })
                    .then(resp => {
            ...
            editEng(item) {
                window.myAjax.post('/MSamplingInspectionRec/EditEng', { seq: item.subEngNameSeq })
                    .then(resp => {

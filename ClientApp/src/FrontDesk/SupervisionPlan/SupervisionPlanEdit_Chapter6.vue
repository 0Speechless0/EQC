<template>
    <div>
        <div class="row justify-content-between">
            ...
            <div v-if="fCanEdit" class="col-12 col-md-5 col-xl-3 mt-3">
                <button v-on:click.stop="newItem()" class="btn btn-shadow btn-outline-secondary btn-block">
                    <i class="fas fa-plus"></i>&nbsp;&nbsp;新增單機設備測試抽驗項目
                </button>
            </div>
        </div>
            ...
            <table class="table table1 min910" border="0">
                ...
                <tbody>
                    <tr v-for="(item, index) in items" v-bind:key="item.id" v-bind:class="{'bg-gray':item.DataType>0}">
                        ...
                        <td>
                            <select :disabled="!fCanEdit" v-model.number="item.EPKind" @change="onInputChange(item)" class="form-control">
        ...
        <FlowChartTpDiagramEdit :title="editDiagramItem.MDName" :editFlowChartTp="editDiagramItem.Seq"
                                :modalShow="modalShow" @handleModalShow="modalShow = !modalShow" type="Chapter6"
    ...
    export default {
        ...
        methods: {
            ...
            onDownloadCheckSheet(item) {
                this.downloadFile('/SupervisionPlan/Chapter6DownloadCheckSheet?engMain=' + this.engMain.Seq + '&seq=' + item.Seq);
            },
            downloadFile(tarUrl) {
            ...
            getList() {
                ...
                window.myAjax.post('/SupervisionPlan/Chapter6', params)
                    .then(resp => {
            ...
            onKeepAllChange() {
                ...
                window.myAjax.post('/SupervisionPlan/Chapter6SaveKeep', { items: this.items })
                    .then(resp => {
            ...
            onInputChange(item) {
                ...
                window.myAjax.post('/SupervisionPlan/Chapter6Save', { item: item })
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            newItem() {
                window.myAjax.post('/SupervisionPlan/Chapter6NewItem', { engMain: this.engMain.Seq })
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            delItem(index, item) {
                //item.edit = false;
                if (confirm('是否確定刪除?')) {
                    window.myAjax.post('/SupervisionPlan/Chapter6Del', { seq: item.Seq })
                        .then(resp => {
                            if (resp.data.result == 0) {
            ...
            onClickSearchEngNo() {
                ...
                this.selCopyItem = -1;
                this.copyItemOptions = [];
                window.myAjax.post('/SupervisionPlan/Chapter6SearchEngNo', { engNo: this.searchEngNo })
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            onClickCopyItem() {
                ...
                window.myAjax.post('/SupervisionPlan/Chapter6CopyItem', { srcId: this.selCopyItem, tarId: this.copyTargetItem.Seq })
                    .then(resp => {
                        if (resp.data.result == 0) {
                            this.getList();
                        }
            ...
            onUploadFlowChartEvent: function (formData) {
                ...
                window.myAjax.post('/SupervisionPlan/Chapter6Upload', formData,
                    {
                        headers: {
            ...
            onDownloadFlowChartEvent(Seq) {
                this.$refs.flowChartModal.downloadFile('/SupervisionPlan/Chapter6DownloadFlowChart?engMain=' + this.engMain.Seq + '&seq=' + Seq);
            },
            onShowFlowChartEvent() {
                this.$refs.flowChartModal.showFile('/SupervisionPlan/Chapter6ShowFlowChart');
            },
            onDelFlowChartEvent() {
                window.myAjax.post('/SupervisionPlan/Chapter6DelFlowChart', { engMain: this.engMain.Seq, seq: this.targetItem.Seq })
                    .then(resp => {
                        if (resp.data.result == 0) {
        ...
        async mounted() {
            console.log('mounted() 第六章 設備運轉測試');
            this.getList();
        }
    }
</script>
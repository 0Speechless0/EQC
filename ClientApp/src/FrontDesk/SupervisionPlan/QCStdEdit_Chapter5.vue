            ...
            <table class="table table1 min1920 tableLayoutFixed" border="0">
                ...
                <tbody>
                    <tr v-for="(item, index) in items" v-bind:key="item.Seq" v-bind:class="{'bg-gray':item.DataType>0}">
                        ...
                        <td>
                            <textarea :disabled="!fCanEdit" rows="3" maxlength="50" type="text" v-model.trim="item.MDTestItem" @change="onDataChange" class="form-control" />
                        </td>
                        <td>
                            <textarea :disabled="!fCanEdit" rows="3" maxlength="100" type="text" v-model.trim="item.MDTestStand1" @change="onDataChange" class="form-control" />
                        </td>
                        <td>
                            <textarea :disabled="!fCanEdit" rows="3" maxlength="100" type="text" v-model.trim="item.MDTestTime" @change="onDataChange" class="form-control" />
                        </td>
                        <td>
                            <textarea :disabled="!fCanEdit" rows="3" maxlength="100" type="text" v-model.trim="item.MDTestMethod" @change="onDataChange" class="form-control" />
                        </td>
                        <td>
                            <textarea :disabled="!fCanEdit" rows="3" maxlength="100" type="text" v-model.trim="item.MDTestFeq" @change="onDataChange" class="form-control" />
                        </td>
                        <td>
                            <textarea :disabled="!fCanEdit" rows="3" maxlength="100" type="text" v-model.trim="item.MDIncomp" @change="onDataChange" class="form-control" />
                        </td>
                        <td>
                            <textarea :disabled="!fCanEdit" rows="3" maxlength="100" type="text" v-model.trim="item.MDManageRec" @change="onDataChange" class="form-control" />
                        </td>
                        <td>
                            <textarea :disabled="!fCanEdit" rows="3" maxlength="255" type="text" v-model.trim="item.MDMemo" @change="onDataChange" class="form-control" />
                        </td>
                    </tr>
                </tbody>
            </table>
...
<script>
    export default {
        ...
        methods: {
            ...
            //下載標準 s20230414
            dnStdDoc() {
                window.comm.dnFile('/QCStdSt/DnStdDoc?mode=5&id=' + this.op1);
            },
            async importTemplate() {
                window.myAjax.post('/QCStdTp/Chapter5_ImportToSt', {
                    code :this.code,
                    seq : this.op1
                })
                    .then(resp => {
            ...
            getList() {
                ...
                window.myAjax.post('/QCStdSt/Chapter5', params)
                    .then(resp => {
            ...
            newItem() {
                window.myAjax.post('/QCStdSt/Chapter5NewItem', { op1: this.op1 })
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            saveItems() {
                for (let item of this.items) {
                    console.log(item.MDType);
                    if (item.OrderNo== null) {
                        alert('[順序] 必須輸入');
                        return;
                    }
                }
                window.myAjax.post('/QCStdSt/Chapter5Save', { items: this.items })
                    .then(resp => {
                        if (resp.data.result == 0) {
                            this.editFlag = false;
                            this.getList();
            ...
            delItem(index, item) {
                //item.edit = false;
                if (confirm('是否確定刪除?')) {
                    window.myAjax.post('/QCStdSt/Chapter5Del', { seq: item.Seq })
                        .then(resp => {
                            if (resp.data.result == 0) {
        ...
        async mounted() {
            console.log('mounted() 第五章 材料設備管制標準');
            this.getList();
        }
    }
</script>
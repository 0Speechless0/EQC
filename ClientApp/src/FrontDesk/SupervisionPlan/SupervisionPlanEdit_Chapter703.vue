<template>
    <div>
        <div class="row justify-content-between">
            ...
            <div v-if="fCanEdit" class="col-12 col-md-5 col-xl-3 mt-3">
                <button v-on:click.stop="newItem()" class="btn btn-shadow btn-outline-secondary btn-block">
                    <i class="fas fa-plus"></i>&nbsp;&nbsp;新增職業安全衛生標準
                </button>
            </div>
        </div>
        ...
        <FlowChartTpDiagramEdit :title="editDiagramItem.MDName" :editFlowChartTp="editDiagramItem.Seq"
                                :modalShow="modalShow" @handleModalShow="modalShow = !modalShow" type="Chapter703"
    ...
    export default {
        ...
        methods: {
            ...
            onDownloadCheckSheet(item) {
                this.downloadFile('/SupervisionPlan/Chapter703DownloadCheckSheet?engMain=' + this.engMain.Seq + '&seq=' + item.Seq);
            },
            downloadFile(tarUrl) {
            ...
            getList() {
                ...
                window.myAjax.post('/SupervisionPlan/Chapter703', params)
                    .then(resp => {
            ...
            onKeepAllChange() {
                ...
                window.myAjax.post('/SupervisionPlan/Chapter703SaveKeep', { items: this.items })
                    .then(resp => {
            ...
            onInputChange(item) {
                ...
                window.myAjax.post('/SupervisionPlan/Chapter703Save', { item: item })
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            newItem() {
                window.myAjax.post('/SupervisionPlan/Chapter703NewItem', { engMain: this.engMain.Seq })
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            delItem(index, item) {
                //item.edit = false;
                if (confirm('是否確定刪除?')) {
                    window.myAjax.post('/SupervisionPlan/Chapter703Del', { seq: item.Seq })
                        .then(resp => {
                            if (resp.data.result == 0) {
            ...
            onClickSearchEngNo() {
                ...
                this.selCopyItem = -1;
                this.copyItemOptions = [];
                window.myAjax.post('/SupervisionPlan/Chapter703SearchEngNo', { engNo: this.searchEngNo })
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            onClickCopyItem() {
                ...
                window.myAjax.post('/SupervisionPlan/Chapter703CopyItem', { srcId: this.selCopyItem, tarId: this.copyTargetItem.Seq })
                    .then(resp => {
                        if (resp.data.result == 0) {
                            this.getList();
                        }
            ...
            onUploadFlowChartEvent: function (formData) {
                ...
                window.myAjax.post('/SupervisionPlan/Chapter703Upload', formData,
                    {
                        headers: {
            ...
            onDownloadFlowChartEvent(Seq) {
                this.$refs.flowChartModal.downloadFile('/SupervisionPlan/Chapter703DownloadFlowChart?engMain=' + this.engMain.Seq + '&seq=' + Seq);
            },
            onShowFlowChartEvent() {
                this.$refs.flowChartModal.showFile('/SupervisionPlan/Chapter703ShowFlowChart');
            },
            onDelFlowChartEvent() {
                window.myAjax.post('/SupervisionPlan/Chapter703DelFlowChart', { engMain: this.engMain.Seq, seq: this.targetItem.Seq })
                    .then(resp => {
                        if (resp.data.result == 0) {

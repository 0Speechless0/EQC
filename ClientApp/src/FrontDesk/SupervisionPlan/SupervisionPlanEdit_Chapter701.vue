<template>
    <div>
        <div class="row justify-content-between">
            ...
            <div v-if="fCanEdit" class="col-12 col-md-5 col-xl-3 mt-3">
                <button v-on:click.stop="newItem()" class="btn btn-shadow btn-outline-secondary btn-block">
                    <i class="fas fa-plus"></i>&nbsp;&nbsp;新增施工抽查項目
                </button>
            </div>
        </div>
                ...
                <tbody>
                    <tr v-for="(item, index) in items" v-bind:key="item.id" v-bind:class="{'bg-gray':item.DataType>0}">
                        ...
                        <td v-if="fCanEdit">
                            ...
                            <!--
                        <button v-if="item.DataType==1" v-on:click.stop="delItem(index, item)" class="mx-1 btn-color9-1 btn btn-block" title="刪除"><i class="fas fa-trash-alt"></i></button>
                        <button v-if="item.DataType==1" data-toggle="modal" data-target="#prepare_edit01" class="mx-1 btn-color9-1 btn btn-block" title="複製"><i class="fas fa-copy"></i></button>
                        -->
                        </td>
                    </tr>
                </tbody>
        ...
        <FlowChartTpDiagramEdit :title="editDiagramItem.MDName" :editFlowChartTp="editDiagramItem.Seq"
                                :modalShow="modalShow" @handleModalShow="modalShow = !modalShow" type="Chapter701"
    ...
    export default {
        ...
        methods: {
            ...
            onDownloadCheckSheet(item) {
                window.comm.dnFile('/SupervisionPlan/Chapter701DownloadCheckSheet?engMain=' + this.engMain.Seq + '&seq=' + item.Seq);
            },
            //清單
            getList() {
                ...
                window.myAjax.post('/SupervisionPlan/Chapter701', params)
                    .then(resp => {
            ...
            onKeepAllChange() {
                ...
                window.myAjax.post('/SupervisionPlan/Chapter701SaveKeep', { items: this.items })
                    .then(resp => {
            ...
            onInputChange(item) {
                ...
                window.myAjax.post('/SupervisionPlan/Chapter701Save', { item: item })
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            newItem() {
                window.myAjax.post('/SupervisionPlan/Chapter701NewItem', { engMain: this.engMain.Seq })
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            delItem(index, item) {
                //item.edit = false;
                if (confirm('是否確定刪除?')) {
                    window.myAjax.post('/SupervisionPlan/Chapter701Del', { seq: item.Seq })
                        .then(resp => {
                            if (resp.data.result == 0) {
            ...
            onClickSearchEngNo() {
                ...
                this.selCopyItem = -1;
                this.copyItemOptions = [];
                window.myAjax.post('/SupervisionPlan/Chapter701SearchEngNo', { engNo: this.searchEngNo })
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            onClickCopyItem() {
                ...
                window.myAjax.post('/SupervisionPlan/Chapter701CopyItem', { srcId: this.selCopyItem, tarId: this.copyTargetItem.Seq })
                    .then(resp => {
                        if (resp.data.result == 0) {
                            this.getList();
                        }
            ...
            onUploadFlowChartEvent: function (formData) {
                ...
                window.myAjax.post('/SupervisionPlan/Chapter701Upload', formData,
                    {
                        headers: {
            ...
            onDownloadFlowChartEvent(Seq) {
                this.$refs.flowChartModal.downloadFile('/SupervisionPlan/Chapter701DownloadFlowChart?engMain=' + this.engMain.Seq + '&seq=' + Seq);
            },
            onShowFlowChartEvent() {
                this.$refs.flowChartModal.showFile('/SupervisionPlan/Chapter701ShowFlowChart');
            },
            onDelFlowChartEvent() {
                window.myAjax.post('/SupervisionPlan/Chapter701DelFlowChart', { engMain: this.engMain.Seq, seq: this.targetItem.Seq })
                    .then(resp => {
                        if (resp.data.result == 0) {
        ...
        async mounted() {
            console.log('mounted() 第七章 701 施工抽查清單範本');
            this.getList();
        }
    }
</script>
            ...
            <table class="table table1 min1920 tableLayoutFixed" border="0">
                ...
                <tbody>
                    <tr v-for="(item, index) in items" v-bind:key="item.Seq" v-bind:class="{'bg-gray':item.DataType>0}">
                        ...
                        <td>
                            <textarea :disabled="!fCanEdit" rows="3" maxlength="100" type="text" v-model.trim="item.EPCheckItem1" @change="onDataChange" class="form-control" />
                        </td>
                        <td>
                            <textarea :disabled="!fCanEdit" rows="3" maxlength="100" type="text" v-model.trim="item.EPStand1" @change="onDataChange" class="form-control" />
                        </td>
                        <td>
                            <textarea :disabled="!fCanEdit" rows="3" maxlength="100" type="text" v-model.trim="item.EPCheckTiming" @change="onDataChange" class="form-control" />
                        </td>
                        <td>
                            <textarea :disabled="!fCanEdit" rows="3" maxlength="100" type="text" v-model.trim="item.EPCheckMethod" @change="onDataChange" class="form-control" />
                        </td>
                        <td>
                            <textarea :disabled="!fCanEdit" rows="3" maxlength="100" type="text" v-model.trim="item.EPCheckFeq" @change="onDataChange" class="form-control" />
                        </td>
                        <td>
                            <textarea :disabled="!fCanEdit" rows="3" maxlength="100" type="text" v-model.trim="item.EPIncomp" @change="onDataChange" class="form-control" />
                        </td>
                        <td>
                            <textarea :disabled="!fCanEdit" rows="3" maxlength="100" type="text" v-model.trim="item.EPManageRec" @change="onDataChange" class="form-control" />
                        </td>
                        <td>
                            <select :disabled="!fCanEdit" v-model.number="item.EPType" @change="onDataChange" class="form-control">
                        ...
                        <!--  td>
                            <div class="custom-control custom-checkbox">
                                <input :disabled="!fCanEdit" type="checkbox" class="custom-control-input" v-bind:id="'AR'+item.Seq" v-model="item.EPMemo" @change="onDataChange">
...
<script>
    export default {
        ...
        methods: {
            ...
            //下載標準 s20230414
            dnStdDoc() {
                window.comm.dnFile('/QCStdSt/DnStdDoc?mode=6&id=' + this.op1);
            },
            async importTemplate() {
                window.myAjax.post('/QCStdTp/Chapter6_ImportToSt', {
                    code :this.code,
                    seq : this.op1
                })
                    .then(resp => {
            ...
            getList() {
                ...
                window.myAjax.post('/QCStdSt/Chapter6', params)
                    .then(resp => {
            ...
            newItem() {
                window.myAjax.post('/QCStdSt/Chapter6NewItem', { op1: this.op1 })
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            saveItems() {
                /*//s20230920
                for (let item of this.items) {
                    console.log(item.EPType);
                    if (item.EPType == null) {
                        alert('[型態] 必須輸入');
                        return;
                    }
                }*/
                window.myAjax.post('/QCStdSt/Chapter6Save', { items: this.items })
                    .then(resp => {
                        if (resp.data.result == 0) {
                            this.editFlag = false;
                            this.getList();
            ...
            delItem(index, item) {
                //item.edit = false;
                if (confirm('是否確定刪除?')) {
                    window.myAjax.post('/QCStdSt/Chapter6Del', { seq: item.Seq })
                        .then(resp => {
                            if (resp.data.result == 0) {
        ...
        async mounted() {
            console.log('mounted() 第六章 設備運轉測試標準');
            this.getList();
        }
    }
</script>
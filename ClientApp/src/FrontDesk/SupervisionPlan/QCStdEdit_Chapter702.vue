            ...
            <table class="table table1 min1920 tableLayoutFixed" border="0">
                ...
                <tbody>
                    <tr v-for="(item, index) in items" v-bind:key="item.Seq" v-bind:class="{'bg-gray':item.DataType>0}">
                        ...
                        <td>
                            <select :disabled="!fCanEdit" v-model.number="item.ECCFlow1" @change="onDataChange" class="form-control">
                        ...
                        <td>
                            <textarea :disabled="!fCanEdit" rows="3" maxlength="100" type="text" v-model.trim="item.ECCCheckItem1" @change="onDataChange" class="form-control" />
                        </td>
                        <td>
                            <textarea :disabled="!fCanEdit" rows="3" maxlength="100" type="text" v-model.trim="item.ECCStand1" @change="onDataChange" class="form-control" />
                        </td>
                        <td>
                            <textarea :disabled="!fCanEdit" rows="3" maxlength="100" type="text" v-model.trim="item.ECCCheckTiming" @change="onDataChange" class="form-control" />
                        </td>
                        <td>
                            <textarea :disabled="!fCanEdit" rows="3" maxlength="100" type="text" v-model.trim="item.ECCCheckMethod" @change="onDataChange" class="form-control" />
                        </td>
                        <td>
                            <textarea :disabled="!fCanEdit" rows="3" maxlength="100" type="text" v-model.trim="item.ECCCheckFeq" @change="onDataChange" class="form-control" />
                        </td>
                        <td>
                            <textarea :disabled="!fCanEdit" rows="3" maxlength="100" type="text" v-model.trim="item.ECCIncomp" @change="onDataChange" class="form-control" />
                        </td>
                        <td>
                            <textarea :disabled="!fCanEdit" rows="3" maxlength="100" type="text" v-model.trim="item.ECCManageRec" @change="onDataChange" class="form-control" />
                        </td>
                        <td>
                            <select :disabled="!fCanEdit" v-model.number="item.ECCType" @change="onDataChange" class="form-control">
                        ...
                        <!--  td>
                            <div class="custom-control custom-checkbox">
                                <input :disabled="!fCanEdit" type="checkbox" class="custom-control-input" v-bind:id="'AR'+item.Seq" v-model="item.ECCMemo" @change="onDataChange">
...
<script>
    export default {
        ...
        methods: {
            ...
            //下載標準 s20230414
            dnStdDoc() {
                window.comm.dnFile('/QCStdSt/DnStdDoc?mode=702&id=' + this.op1);
            },
            ...
            getList() {
                ...
                window.myAjax.post('/QCStdSt/Chapter702', params)
                    .then(resp => {
            ...
            newItem() {
                window.myAjax.post('/QCStdSt/Chapter702NewItem', { op1: this.op1 })
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            saveItems() {
                for (let item of this.items) {
                    console.log(item.ECCType);
                    if (/*item.ECCType == null ||*/ item.ECCFlow1 == null) { //s20230920
                        alert('[流程] 必須輸入');
                        return;
                    }
                }
                window.myAjax.post('/QCStdSt/Chapter702Save', { items: this.items })
                    .then(resp => {
                        if (resp.data.result == 0) {
                            this.editFlag = false;
                            this.getList();
            ...
            delItem(index, item) {
                //item.edit = false;
                if (confirm('是否確定刪除?')) {
                    window.myAjax.post('/QCStdSt/Chapter702Del', { seq: item.Seq })
                        .then(resp => {
                            if (resp.data.result == 0) {
        ...
        async mounted() {
            console.log('mounted() 第七章 702 環境保育標準');
            this.getList();
        }
    }
</script>
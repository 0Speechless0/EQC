<template>
    <div>
        <h5 class="insearch mt-0 py-2">施工抽查紀錄</h5>
            ...
            <table class="table table-responsive-md">
                <thead class="insearch">
                    <tr>
                        ...
                        <th class="text-right"><strong>抽查紀錄數</strong></th>
                        <th class="text-right"><strong>缺失個數</strong></th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="(item, index) in items" v-bind:key="item.Seq">
                        ...
                        <td class="text-right">{{item.constCheckRecCount}}</td>
                        <td class="text-right">{{item.missingCount}}</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
<script>
    export default {
        ...
        methods: {
            getList() {
                this.items = [];
                window.myAjax.post('/EngHistoryInfo/GetConstructionCheckList', {

<template>
    <div>
        <h5 class="insearch mt-0 py-2">重大事故資料</h5>
            ...
            <table class="table table-responsive-md">
                <thead class="insearch">
                    <tr>
                        ...
                        <th class="text-right"><strong>死亡</strong></th>
                        <th class="text-right"><strong>重傷</strong></th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="(item, index) in items" v-bind:key="item.Seq">
                        ...
                        <td class="text-right">{{item.WSDeadCnt}}</td>
                        <td class="text-right">{{item.WSHurtCnt}}</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
<script>
    export default {
        ...
        methods: {
            getList() {
                this.items = [];
                window.myAjax.post('/EngHistoryInfo/GetWorkSafetyTribunalList', {

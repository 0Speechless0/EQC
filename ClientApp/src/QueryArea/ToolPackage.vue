            ...
            <table class="table table-responsive-md table-hover">
                ...
                <tbody>
                    <tr v-for="(item, index) in items" v-bind:key="item.Seq" v-bind:class="{'btn-color11-1':item==selectItem}">
                        ...
                        <td>
                            <div class="d-flex">
                                <button v-on:click.stop="openModal(item.Seq)" class="btn btn-color11-3 btn-xs"><i class="fas fa-briefcase"> 工具包</i></button>
                            </div>
                        </td>
                        <toolPackageModal :engSeq="item.Seq"></toolPackageModal>
                    </tr>
                </tbody>
            </table>
        </div>
...
<script>
import toolPackageModal from "../components/toolPackageModal.vue";
    export default {
        components :{
            toolPackageModal
        },
        ...
        methods: {
            openModal(engSeq)
            {   
                window.openModal('#toolPackageModal'+engSeq);
            },
            getStateCss(state) {

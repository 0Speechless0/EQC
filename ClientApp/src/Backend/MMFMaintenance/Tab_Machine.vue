<template>
    <div>
        <h5 style="margin: inherit;">機具維護功能</h5><p></p>
        <div class="row d-flex bd-highlight mb-3" style="padding-left:10px">
            ...
            <a :href="'ExcelTemplate/材料機具燃料維護-機具.xlsx'" class="bd-highlight" download style="padding-left: 15px;">
            ...
            <table class="table table-responsive-md table-hover VA-middle">
                ...
                <tbody>
                    <tr v-if="fAddItem">
                        ...
                        <td><input v-model.number="newItem.ConsumptionRate" type="number" class="form-control"></td>
                        <td><input v-model.trim="newItem.ConsumptionRateUnit" maxlength="30" type="text" class="form-control"></td>
                        <td><input v-model.trim="newItem.FuelKind" maxlength="30" type="text" class="form-control"></td>
                        <td><input v-model.number="newItem.FuelKgCo2e" type="number" class="form-control"></td>
                        <td><input v-model.trim="newItem.FuelUnit" maxlength="30" type="text" class="form-control"></td>
                    ...
                    <tr v-for="(item, index) in items" v-bind:key="item.Seq">
                        ...
                        <template v-if="item.Seq == editSeq">
                            ...
                            <td><input v-model.number="editRecord.ConsumptionRate" type="number" class="form-control"></td>
                            <td><input v-model.trim="editRecord.ConsumptionRateUnit" maxlength="30" type="text" class="form-control"></td>
                            <td><input v-model.trim="editRecord.FuelKind" maxlength="50" type="text" class="form-control"></td>
                            <td><input v-model.number="editRecord.FuelKgCo2e" type="number" class="form-control"></td>
                            <td><input v-model.trim="editRecord.FuelUnit" maxlength="30" type="text" class="form-control"></td>
...
<script>
    export default {
        ...
        methods: {
            //下載碳排表
            download() {
                window.myAjax.get('/MMFMaintenance/dnMachine', { responseType: 'blob' })
                    .then(resp => {
            ...
            onNewRecord() {
                window.myAjax.post('/MMFMaintenance/NewMachineRecord')
                    .then(resp => {
            ...
            onDelRecord(item) {
                ...
                if (confirm('是否確定刪除資料？')) {
                    window.myAjax.post('/MMFMaintenance/DelMachineRecord', { id: item.Seq })
                        .then(resp => {
                            if (resp.data.result == 0) {
                                this.getResords();
                            }
            ...
            onSaveRecord(uItem) {
                ...
                window.myAjax.post('/MMFMaintenance/UpdateMachineRecord', { m: uItem })
                    .then(resp => {
                        if (resp.data.result == 0) {
                            this.editSeq = -99;
                            this.getResords();
            ...
            upload(uploadfiles) {
                window.myAjax.post('/MMFMaintenance/MachineUpload', uploadfiles,
                    {

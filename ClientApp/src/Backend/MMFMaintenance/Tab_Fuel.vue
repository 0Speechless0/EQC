<template>
    <div>
        <h5 style="margin: inherit;">燃料維護功能</h5><p></p>
        <div class="row d-flex bd-highlight mb-3" style="padding-left:10px">
            ...
            <a :href="'ExcelTemplate/材料機具燃料維護-燃料.xlsx'" class="bd-highlight" download style="padding-left: 15px;">
...
<script>
    export default {
        ...
        methods: {
            //下載碳排表
            download() {
                window.myAjax.get('/MMFMaintenance/dnFuel', { responseType: 'blob' })
                    .then(resp => {
            ...
            onNewRecord() {
                window.myAjax.post('/MMFMaintenance/NewFuelRecord')
                    .then(resp => {
            ...
            onDelRecord(item) {
                ...
                if (confirm('是否確定刪除資料？')) {
                    window.myAjax.post('/MMFMaintenance/DelFuelRecord', { id: item.Seq })
                        .then(resp => {
                            if (resp.data.result == 0) {
                                this.getResords();
                            }
            ...
            onSaveRecord(uItem) {
                //console.log(uItem);
                if (this.strEmpty(uItem.NameSpec) || this.strEmpty(uItem.Unit) || !window.comm.isNumber(uItem.KgCo2e) ) {
                    alert('電力/燃料名稱,單位碳排量,單位 必須輸入!');
                    return;
                }
                window.myAjax.post('/MMFMaintenance/UpdateFuelRecord', { m: uItem })
                    .then(resp => {
                        if (resp.data.result == 0) {
                            this.editSeq = -99;
                            this.getResords();
            ...
            getResords() {
                this.fAddItem = false;
                this.items = [];
                window.myAjax.post('/MMFMaintenance/GetFuelRecords', {
            ...
            upload(uploadfiles) {
                window.myAjax.post('/MMFMaintenance/FuelUpload', uploadfiles,
                    {

    ...
    <div>
        ...
        <div class="row justify-content-between">
            ...
            <div class="col-12 col-md-4 col-xl-3 mt-3">
                <button v-bind:disabled="!showAdd" v-on:click.stop="newItem()" v-bind:class="{'btn-shadow btn-color1':showAdd, 'bg-gray':!showAdd}" role="button" class="btn btn-block">
                    <i class="fas fa-plus"></i>&nbsp;&nbsp;新增品質計畫書
                </button>
            </div>
        </div>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header bg-R text-white">
                        <h6 class="modal-title">上傳品質計畫書</h6>
                    </div>
    ...
    export default {
        ...
        methods: {
            ...
            upload() {
                ...
                window.myAjax.post('/QualityPlanTp/Upload', files,
                    {
                        headers: {
            ...
            download(item) {
                ...
                window.myAjax.get('/QualityPlanTp/Download?seq='+item.Seq, { responseType: 'blob' })
                    .then(resp => {
            ...
            getList() {
                this.loading = true;
                window.myAjax.post('/QualityPlanTp/GetList').then(resp => {
                    this.loading = false;
            ...
            newItem() {
                this.loading = true;
                window.myAjax.post('/QualityPlanTp/Add').then(resp => {
                    this.loading = false;
                    if (resp.data.result == 0) {
                        this.getList();
                    } else {

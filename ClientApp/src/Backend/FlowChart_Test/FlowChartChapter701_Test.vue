            ...
            <table class="table table3 min910" border="0">
                ...
                <tbody>
                    ...
                    <tr name="tmpRow" style="display: none;">
                        <td colspan="5" style="background: #f4f5f6;"><qcBase701  v-bind:op1="QCSseq" :op2="QCSopenclose"></qcBase701></td>
                    </tr>
                </tbody>
            </table>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            {
                let {data} = await window.myAjax.post("FlowChartTp/getFlowChartTpDiagramJson", {id: item.Seq, type:"Chapter701"});
                console.log(data);
            ...
            upload() {
                ...
                window.myAjax.post('/FlowChartTp_TestWeb/Chapter701Upload', files,
                    {
                        headers: {
            ...
            download(item) {
                //window.open('/FlowChartTp/Chapter701Download' + '?seq=' + item.Seq);
                window.myAjax.get('/FlowChartTp_TestWeb/Chapter701Download?seq=' + item.Seq, { responseType: 'blob' })
                    .then(resp => {
            ...
            Cekdownload(item) {
                this.Wordphoto = '';
                window.myAjax.post('/FlowChartTp_TestWeb/Chapter701ShowWord', { seq: item.Seq })
                    .then(resp => {
            ...
            getList() {
                ...
                window.myAjax.post('/FlowChartTp_TestWeb/Chapter701', params)
                    .then(resp => {
            ...
            newItem() {
                if (this.editFlag) return;
                window.myAjax.post('/FlowChartTp/Chapter701NewItem')
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            saveItem(item) {
                ...
                window.myAjax.post('/FlowChartTp/Chapter701Save', { item: item })
                    .then(resp => {
                        if (resp.data.result == 0) {
                            item.edit = false;
                            this.editFlag = false;
            ...
            delItem(index, item) {
                ...
                if (confirm('是否確定刪除?')) {
                    window.myAjax.post('/FlowChartTp/Chapter701Del', { seq: item.Seq })
                        .then(resp => {
                            if (resp.data.result == 0) {
            ...
            showPicture(item) {
                window.myAjax.post('/FlowChartTp_TestWeb/Chapter701Show', { seq: item.Seq })
                    .then(resp => {
                        if (resp.data.url != null) {
                            console.log(resp.data.url);
        ...
        async mounted() {
            console.log('mounted() 第七章 施工抽查清單範本');
            this.getList();
        }
    }
</script>
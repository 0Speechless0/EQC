            ...
            $(go.Diagram, this.DiagramId,
                {
                    ...
                    SelectionDeleting: function (e) {
                        ...
                        var targetNodesInLevel = e
                            .diagram.model.nodeDataArray
                            .filter(node => node.level == deleteNode.data.level)
                            .filter(node => {
                              console.log(node);
                              var parentNode = e.diagram.findNodeForKey(node.key).findNodesInto().first();
                              var _parentNode = e.diagram.findNodeForKey(deleteNode.key).findNodesInto().first();
                              if(parentNode != null && _parentNode != null && deleteNode.key != node.key)
                                return parentNode.key == _parentNode.key
                              else return false;

                            });
                        targetNodesInLevel.sort((a,b) => a.no > b.no ? 1 : -1);
                        
                        targetNodesInLevel.forEach((node, index) => {
                                node.no = deleteNode.no < node.no ? index : index + 1 ;
        ...
        myDiagram.nodeTemplate =  // the default node template
            $(go.Node, "Table",
                ...
                $(go.Panel, "Auto",
                    ...
                    $(go.TextBlock, "center",
                        {
                            stroke: "black", font: "16px sans-serif", editable: false,
                            isMultiline: false,

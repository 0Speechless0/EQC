                    ...
                    <tr v-for="(item, index) in items" v-bind:key="item.id">
                        ...
                        <td>
                            <div class="row justify-content-center m-0">
                                ...
                                <!-- <a v-if="item.FlowCharOriginFileName && item.FlowCharOriginFileName.length>0" href="#" v-on:click="showPicture(item)" class="btn btn-color11-2 btn-xs m-1" title="下載" data-toggle="modal" data-target="#exampleModal"><i class="fas fa-eye"></i> 預覽</a> -->
                            </div>
                            <!-- 大圖 -->
                            <div class="modal fade" id="exampleModal" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="exampleModalLabel" aria-modal="true" aria-hidden="true">
...
<script>
import FlowChartTpDiagramEdit from './FlowChartTpDiagramEdit';
    export default {
    props: ["addItem"],
    watch: {
    ...
    methods: {
        ...
        upload(seq, file) {
            ...
            window.myAjax.post("/FlowChartTp/Chapter5Upload", this.files, {
                headers: {
            ...
            //         this.targetItem.FlowCharOriginFileName = file.name;
        ...
        download(Seq) {
            ...
            window.myAjax.get("/FlowChartTp/Chapter5Download?seq=" + Seq, { responseType: "blob" })
                .then(resp => {
        ...
        getList() {
            ...
            window.myAjax.post("/FlowChartTp/Chapter5", params)
                .then(resp => {
        ...
        newItem() {
            if (this.editFlag)
                return;
            window.myAjax.post("/FlowChartTp/Chapter5NewItem")
                .then(resp => {
                if (resp.data.result == 0) {
        ...
        saveItem(item) {
            if (item.OrderNo === "" || window.comm.stringEmpty(item.ExcelNo) || window.comm.stringEmpty(item.MDName)) {
                alert("所有欄位均須輸入資料");
                return;
            }
            window.myAjax.post("/FlowChartTp/Chapter5Save", { item: item })
                .then(resp => {
                if (resp.data.result == 0) {
                    item.edit = false;
                    this.editFlag = false;
        ...
        delItem(index, item) {
            ...
            if (confirm("是否確定刪除?")) {
                window.myAjax.post("/FlowChartTp/Chapter5Del", { seq: item.Seq })
                    .then(resp => {
                    if (resp.data.result == 0) {
                        if (item.edit)
                            this.editFlag = false;
        ...
        //圖片預覽url
        showPicture(item) {
            window.myAjax.post("/FlowChartTp/Chapter5Show", { seq: item.Seq })
                .then(resp => {
    ...
    components: { FlowChartTpDiagramEdit }
}
</script>
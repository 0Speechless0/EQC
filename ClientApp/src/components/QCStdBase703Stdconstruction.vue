    ...
    <div>
        ...
        <div v-if="users.length > 0">
            <div style="float:right">
                <button v-on:click.stop="newItem()">+ Add 703</button>
            </div>
...
<script>
    export default {
        ...
        methods: {
            async getList() {
                this.loading = true;
                const { data } = await window.myAjax.post('/QCStdBase/Chapter703', { op1: this.op1});
                this.users = data;
                this.loading = false;
            },
            newItem() {
        ...
        async mounted() {
            console.log('mounted() 第七章 703 環境保育管理標準');
        }
    }
</script>
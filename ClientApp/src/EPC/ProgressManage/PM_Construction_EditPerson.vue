...
<script>
    export default {
        ...
        methods: {
            onNewRecord() {
                window.myAjax.post('/EPCProgressManage/NewPersonRecord')
                    .then(resp => {
            ...
            onDelRecord(item) {
                ...
                if (confirm('是否確定刪除資料？')) {
                    window.myAjax.post('/EPCProgressManage/DelPersonRecord', { id: item.Seq })
                        .then(resp => {
                            if (resp.data.result == 0) {
                                this.getResords();
                            }
            ...
            onSaveRecord(uItem) {
                ...
                window.myAjax.post('/EPCProgressManage/UpdatePersonRecords', { m: uItem })
                    .then(resp => {
                        if (resp.data.result == 0) {
                            this.editSeq = -99;
                            this.getResords();
            ...
            getResords() {
                this.items = [];
                window.myAjax.post('/EPCProgressManage/GetPersonRecords', { id: this.supDailyItem.Seq})
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            getMaterialOptions() {
                this.items = [];
                window.myAjax.post('/EPCProgressManage/GetPersonOptions')
                    .then(resp => {

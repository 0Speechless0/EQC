...
<script>
    export default {
        ...
        methods: {
            onNewRecord() {
                window.myAjax.post('/ECProgressManage/NewMaterialRecord')
                    .then(resp => {
            ...
            onDelRecord(item) {
                ...
                if (confirm('是否確定刪除資料？')) {
                    window.myAjax.post('/ECProgressManage/DelMaterialRecord', { id: item.Seq })
                        .then(resp => {
                            if (resp.data.result == 0) {
                                this.getResords();
                            }
            ...
            onSaveRecord(uItem) {
                ...
                window.myAjax.post('/ECProgressManage/UpdateMaterialRecords', { m: uItem })
                    .then(resp => {
                        if (resp.data.result == 0) {
                            this.editSeq = -99;
                            this.getResords();
            ...
            getResords() {
                this.items = [];
                window.myAjax.post('/ECProgressManage/GetMaterialRecords', { id: this.supDailyItem.Seq})
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            getMaterialOptions() {
                this.items = [];
                window.myAjax.post('/ECProgressManage/GetMaterialOptions')
                    .then(resp => {
        ...
        mounted() {
            console.log('mounted 工程變更-工地材料管理概況');

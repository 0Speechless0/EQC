<template>
    <div>
        ...
        <!-- div class="form-row">
            <div class="col-2 form-inline my-2">
                <label class="my-2 mx-2">決標日期</label>
            </div>
            <div class="col-3 form-inline my-2">
                <b-input-group>
                    ...
                    <b-form-datepicker v-model="chsAwardDate" :hide-header="true"
                                       button-only right size="sm" @context="onDatePicketChange($event, 'AwardDate')">
                    </b-form-datepicker>
                </b-input-group>
            </div>
            <div class="col-1 my-2">
                <button v-on:click.stop="updateData()" role="button" class="btn btn-color11-4 btn-block mx-1">
                    <i class="fas fa-save"> 儲存</i>
                </button>
            </div>
        </div -->
        ...
        <ul class="nav nav-tabs" role="tablist">
            <li class="nav-item">
                <a @click="onSetMidKine" class="nav-link" data-toggle="tab" href="#menu00">設定中類</a>
            </li>
            <template v-for="(item, index) in dateList">
                <li v-if="showDateListTab" v-bind:key="item.AdjMonthStr" class="nav-item">
                    <a @click="onChangeDate(item)" class="nav-link" data-toggle="tab" href="#menu01">{{item.AdjMonthStrYM}}</a>
                </li>
            </template>
        </ul>
                ...
                <h5 class="insearch mt-0 py-2">
                    <div class="form-row mb-1">
                        <div class="table-responsive tableFixHead">
                            ...
                            <table class="table table-responsive-md table-hover VA-middle">
                                ...
                                <tbody>
                                    <tr v-for="(item, index) in midKinds" v-bind:key="item.Seq" class="bg-1-30">
                                        <td class="text-center"><strong>{{index+1}}</strong></td>
                                        <td style="font-size: 15px;">{{item.Description}}({{item.ItemCode}})</td>
                                        <td>{{item.Amount}}</td>
                                        <td>
                                            <select v-model="item.PriceIndexKindId" class="form-control">
                    ...
                    <div v-if="midKinds.length > 0" class="d-flex justify-content-center">
                        <div class="col-12 col-sm-4 col-xl-2 my-2">
                            <button @click="saveMidKind" role="button" class="btn btn-shadow btn-block btn-color11-4"> 儲存 <i class="fas fa-save"></i></button>
                        </div>
                    </div>
                </h5>
            </div>
        ...
        <div v-if="itemGroup" class="card whiteBG mb-4 pattern-F colorset_1">
            <div class="form-row" role="toolbar">
                <div class="col-12 col-sm-6 col-md-auto mb-3 mb-sm-0 mt-sm-2 mt-md-0">
                    <button @click="onCalPriceIndexClick" type="button" class="btn btn-outline-secondary btn-sm" title="重新計算">重新計算 &nbsp;<i class="fas fa-check"></i></button>
                </div>
            </div>
            <div class="card-body">
                <h5 class="insearch mt-0 py-2">
...
<script>
    export default {
        ...
        methods: {
            ...
            getEngMain() {
                this.engMain = {};
                window.myAjax.post('/EPCPriceAdjustment/GetEngItem', { id: this.tenderItem.Seq })
                    .then(resp => {
            ...
            updateAwardDate() {
                ...
                window.myAjax.post('/EPCPriceAdjustment/UpdateEngAwardDate', { engMain: this.engMain })
                    .then(resp => {
                        if (resp.data.result == 0) {
                            this.getEngMain();
                            alert(resp.data.message);
            ...
            saveMidKind() {
                ...
                window.myAjax.post('/EPCPriceAdjustment/SaveMidKind', { items: this.midKinds })
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            //檢查中類是否設定
            checkModKind() {
                if (this.midKinds.length == 0) { return false; }
                var i = 0;
            ...
            onCalPriceIndexClick() {
                ...
                window.myAjax.post('/EPCPriceAdjustment/CalPriceIndex', { id: this.tenderItem.Seq, tarDate: this.activeItem.AdjMonthStr })
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            onChangeDate(item) {
                ...
                this.itemGroup = null;
                window.myAjax.post('/EPCPriceAdjustment/GetList', { id: this.tenderItem.Seq, tarDate: item.AdjMonthStr })
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            getDateList() {
                this.dateList = [];
                window.myAjax.post('/EPCPriceAdjustment/GetDateList', { id: this.tenderItem.Seq })
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            //工程變更
            onEngChangeClick() {
                window.myAjax.post('/EPCPriceAdjustment/CheckEngChangeDate', { id: this.tenderItem.Seq })
                    .then(resp => {
                        if (resp.data.result == 0) {
                            this.$swal({
                                title: '工程變更, 是否確定?',
            ...
            execChangeConfirm(result) {
                ...
                window.myAjax.post('/EPCPriceAdjustment/ExecEngChangeDate', { id: this.tenderItem.Seq })
                    .then(resp => {
                        if (resp.data.result == 0) {
                            this.getDateList();
                        }

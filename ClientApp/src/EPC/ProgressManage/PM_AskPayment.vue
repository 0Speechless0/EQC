    ...
    export default {
        ...
        methods: {
            ...
            onAddAskPayment() {
                ...
                window.myAjax.post('/EPCAskPayment/AddAskPayment', { id: this.tenderItem.Seq, tarDate: this.addDate })
                    .then(resp => {
            ...
            getDateList() {
                ...
                this.items = [];
                window.myAjax.post('/EPCAskPayment/GetDateList', { id: this.tenderItem.Seq })
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            onChangeDate(item) {
                this.activeItem = null;
                this.items = [];
                window.myAjax.post('/EPCAskPayment/GetList', { id: item.Seq })
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            onDelItem() {
                if (confirm('刪除 請款日:' + this.activeItem.ItemDate + ' 資料?\n是否確定? ')) {
                    window.myAjax.post('/EPCAskPayment/DelAPItem', { id: this.activeItem.Seq })
                        .then(resp => {
                            if (resp.data.result == 0) {
                                this.getDateList();
                            }
            ...
            fillCompleted() {
                if (confirm('請款日:' + this.activeItem.ItemDate + '已完成請款, 之後將不能修改予刪除\n是否確定? ')) {
                    window.myAjax.post('/EPCAskPayment/APCompleted', { id: this.activeItem.Seq })
                        .then(resp => {
                            if (resp.data.result == 0) {
                                this.getDateList();
                            }
            ...
            //儲存
            onSaveClick() {
                window.myAjax.post('/EPCAskPayment/APSave', {
            ...
            //下載請款調整
            downloadAdj() {
                this.download('/EPCAskPayment/DownloadAdj?id=' + this.activeItem.Seq);
            },
            //下載估驗請款單
            downloadInvoice() {
                this.download('/EPCAskPayment/DownloadInvoice?id=' + this.activeItem.Seq);
            },
            download(action) {
            ...
            upload(uploadfiles, action) {
                window.myAjax.post('/EPCAskPayment/' + action, uploadfiles,
                    {

        ...
        <div>
            ...
            <div v-if="supDailyItem != null" class="card-footer">
                <div class="row justify-content-center">
                    ...
                    <div class="col-12 col-sm-4 col-xl-2 pt-1">
...
<script>
    ...
    export default {
        ...
        components: {
            ...
            EditMaterial: require('./PM_EngChange_Construction_EditMaterial').default,
            EditPerson: require('./PM_EngChange_Construction_EditPerson').default,
            EditEquipment: require('./PM_EngChange_Construction_EditEquipment').default,
        },
        methods: {
            //複製前日材料,人員,機具資料 s20231116
            copyMiscData() {
                this.supDailyItem.ItemDate = this.supDailyItem.ItemDateStr;
                window.myAjax.post('/ECProgressManage/CopyConstructionMiscData', { supDailyItem: this.supDailyItem })
                    .then(resp => {
            ...
            //下載 施工日報 s20230831
            downloadDaily() {
                let action = '/ECProgressManage/DownloadCDaily?id=' + this.supDailyItem.Seq + '&eEM=' + this.dnDailyMode;
                window.comm.dnFile(action, this.dnCallBack);
            },
            //s20230831
            dnCallBack(result) {
            ...
            uploadMultiDailyLog(event) {
                ...
                window.myAjax.post('/ECProgressManage/UploadMultiDailyLog', uploadfiles,
                    {
            ...
            //下載 Excel範本
            download() {
                let action = '/ECProgressManage/Download?id=' + this.supDailyItem.Seq;// + '&engId=' + this.supDailyItem.EngMainSeq;
                window.comm.dnFile(action);
            },
            
            //匯入日誌(excel)
            /*uploadDailyLog(event) {
                var files = event.target.files || event.dataTransfer.files;
                // 預防檔案為空檔
                if (!files.length) return;
                if (!files[0].type.match('application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')) {// && !files[0].type.match('application/vnd.ms-excel') ) {
                ...
                var uploadfiles = new FormData();
                uploadfiles.append("id", this.supDailyItem.Seq);
                uploadfiles.append("file", files[0], files[0].name);
                window.myAjax.post('/ECProgressManage/UploadDailyLog', uploadfiles,
                    {
                        headers: { 'Content-Type': 'multipart/form-data' }
                    }).then(resp => {
                        if (resp.data.result == 0) {
            ...
            fillCompleted() {
                ...
                if (confirm('填報完成後將不能再修改\n是否確定? ')) {
                    setTimeout(
                        window.myAjax.post('/ECProgressManage/DailyLogCompleted', { id: this.supDailyItem.Seq })
                            .then(resp => {
                                if (resp.data.result == 0) {
            ...
            getConstructionItem(sDay) {
                ...
                this.selectTab = '';
                if (sDay == null) return;
                window.myAjax.post('/ECProgressManage/GetConstructionItem', { id: this.tenderItem.Seq, tarDate: sDay })
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            getConstructionCalInfo() {
                window.myAjax.post('/ECProgressManage/GetConstructionInfo', { id: this.tenderItem.Seq, fromDate: this.fromDate })
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            onCalDayClick(day) {
                ...
                //s20230412
                window.myAjax.post('/ECProgressManage/CheckActiveDate', { id: this.tenderItem.Seq, tarDate: day.id })
                    .then(resp => {
                        if (resp.data.result == 0) {
                            this.planItemsTotal = 0;
                            this.getConstructionItem(day.id);

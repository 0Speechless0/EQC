...
<script>
    export default {
        ...
        methods: {
            ...
            onNewRecord() {
                this.clearSelItem();
                window.myAjax.post('/EPCProgressManage/NewEquipmentRecord')
                    .then(resp => {
            ...
            onDelRecord(item) {
                ...
                if (confirm('是否確定刪除資料？')) {
                    window.myAjax.post('/EPCProgressManage/DelEquipmentRecord', { id: item.Seq })
                        .then(resp => {
                            if (resp.data.result == 0) {
                                this.getResords();
                            }
            ...
            onSaveRecord(uItem) {
                ...
                window.myAjax.post('/EPCProgressManage/UpdateEquipmentRecords', { m: tarItem })
                    .then(resp => {
                        if (resp.data.result == 0) {
                            this.editSeq = -99;
                            this.getResords();
            ...
            getResords() {
                this.items = [];
                window.myAjax.post('/EPCProgressManage/GetEquipmentRecords', { id: this.supDailyItem.Seq})
                    .then(resp => {
                        if (resp.data.result == 0) {
            ...
            getEquipmentOptions() {
                this.items = [];
                window.myAjax.post('/EPCProgressManage/GetEquipmentOptions')
                    .then(resp => {
            ...
            getEquipmentSpecOptions(kind) {
                this.clearSelItem();
                this.items = [];
                window.myAjax.post('/EPCProgressManage/GetEquipmentSpecOptions', { kind:kind })
                    .then(resp => {

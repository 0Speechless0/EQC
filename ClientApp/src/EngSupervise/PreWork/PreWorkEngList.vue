...
<template>
    <div>
            ...
            <table class="table table-responsive-md table-hover">
                ...
                <tbody>
                    <tr v-for="(item, index) in targetPhaseItems" v-bind:key="item.Seq">
                        ...
                        <td>
                            <i v-bind:class="getStateCss(item.ExecState)"></i>{{ item.ExecState }}
                            <div v-if="item.Updated" style="color:blue">
                                (弱面更新)
                            </div>
                        </td>
                        <td>
                            <div class="d-flex">
                                <button @click="onEditEng(item,2)" class="btn btn-color11-3 btn-xs sharp mr-1"
                                        data-toggle="modal" data-target="#prepare_change01" title="移動期別">
                                    <i class="fas fa-exchange-alt"></i>
                                </button>
                                <button @click="onEditEng(item,1)" class="btn btn-color11-1 btn-xs sharp mr-1"
                                        data-toggle="modal" data-target="#prepare_edit01" title="編輯">
        ...
        <div class="modal fade" id="prepare_edit01">
            <div v-if="targetPhase != null && tarItem" class="modal-dialog modal-xl modal-dialog-centered ">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-center">
                                                <button @click="addOutCommitte(item)" v-if="item.mode == 1"
                                                        class="btn btn-color11-1 btn-xs sharp mx-1" title="加入">
                        ...
                        <div class="d-flex">
                            <h5>備註：</h5>
                            <div class="row pt-3 pl-3 align-items-center " v-if="tarItem.Updated">
                                <button @click="syncWeakness(tarItem)" type="button" class="btn btn-color11-3 btn-xs">同步弱面 <i class="fas fa-sync"></i></button>
                            </div>
        ...
        <!-- 小視窗 移動期別 -->
        <div class="modal fade" id="prepare_change01">
            <div v-if="targetPhase != null && tarItem2" class="modal-dialog modal-xl modal-dialog-centered ">
                <div class="modal-content">
                    <div class="modal-header bg-3 text-white">
                        <h6 class="modal-title font-weight-bold">移動工程期別</h6>
                        <button id="btnCloseEditModal2" type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <h5 class="mt-0 insearch">
                            工程編號：{{ tarItem2.EngNo }} &nbsp; 工程名稱：{{ tarItem2.EngName }} &nbsp;
                            督導期別：{{ targetPhase.PhaseCode }}
                        </h5>
                        <h5>
                            <div class="form-inline">
                                <label class="my-1 mr-2" for="people_A">移動期別：</label>
                                <select v-model="changePhaseItem" class="form-control my-1 mr-sm-2">
                                    <option v-for="option2 in yearPhaseItems" v-bind:value="option2.Seq"
                                            v-bind:key="option2.PhaseCode" v-bind:selected="option2.PhaseCode == targetPhase.PhaseCode">
                                        {{ option2.PhaseCode }}
                                    </option>
                                </select>
                            </div>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-color3" data-dismiss="modal">關閉</button>
                        <button @click="onSavechangeEng" type="button" class="btn btn-color11-1">
                            儲存 <i class="fas fa-save"></i>
                        </button>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        syncWeakness(item)
        {
            window.myAjax.post('/ESPreWork/SyncWeakness', {prjSeq : item.PrjXMLSeq, pharseSeq :this.targetPhase.Seq})
                .then(resp => {
                    if(resp.data )
                        item.Memo = resp.data;
                ...
                .then(resp => {
                    if (resp.data.result == 0) {
                        if (type == 1) {
                            ...
                            this.tarItem.PrjXMLSeq = item.PrjXMLSeq;
                            this.tarItem.Updated = item.Updated;
                            this.getOutCommitte('');
                            ...
                            this.tarItem2 = resp.data.item;
                            this.tarItem2.PrjXMLSeq = item.PrjXMLSeq;
                            this.tarItem2.Updated = item.Updated;
                            this.changePhaseItem = this.targetPhase.Seq;
                        }
                    } else {
                        alert(resp.data.msg);
                    }
        ...
        onSavechangeEng() {
            window.myAjax.post('/ESPreWork/SaveChangeEng', { m: this.tarItem2, PhaseSeq: this.changePhaseItem})
                .then(resp => {
                    alert(resp.data.msg);
                    if (resp.data.result == 0) {
                        document.getElementById('btnCloseEditModal2').click();
                        this.onSearchPhase();
                    }
                })
                .catch(err => {
